...
class Yahtzee:
    ...
    def is_it_low_roll(self):
        return (np.all(self.rolls<=2))         

    def is_it_three_of_a_kind(self):       
        if np.any(self.count_outcomes()==3):
            return np.any(self.count_outcomes()==1)
            
    def is_it_large_straight(self):
        counter = 0
        game = np.sort(self.rolls)
        
        for match in range(len(self.rolls)-1):
            if game[match] + 1 == game[match + 1]:
                counter += 1
                if counter == 4:
                    return True
